...
The `mark-base-demo-vault/` directory is a ready-to-use Obsidian vault. When adding new plugin features:

- Add representative sample notes to `mark-base-demo-vault/` demonstrating the feature.
- Update `mark-base-demo-vault/README.md` if new files require explanation.
- Do **not** commit `mark-base-demo-vault/.obsidian/plugins/` build artifacts — those are regenerated by `scripts/build-to-demo-vault.ps1`.
